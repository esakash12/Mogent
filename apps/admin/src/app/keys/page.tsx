...
export default function ApiKeysPage() {
  ...
  const fetchKeys = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/keys`, { cache: "no-store" });
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={fetchKeys}
            className="px-3 py-1.5 rounded-lg bg-[#111] hover:bg-[#222] border border-[#222] text-xs font-mono text-[#888] hover:text-[#EDEDED] flex items-center gap-2 transition-colors cursor-pointer"
          >
      ...
      <form onSubmit={handleAddKey} className="p-4 rounded-xl border border-[#222] bg-[#0A0A0A] flex flex-col sm:flex-row items-center gap-3">
        ...
        <button
          ...
          className="w-full sm:w-auto px-5 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black font-semibold text-[13px] flex items-center justify-center gap-2 transition-colors shrink-0 cursor-pointer"
        >
          {isAdding ? (
